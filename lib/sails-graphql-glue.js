...
const actionAsResolver = (action) => {
  ...
  return async (parent, args, context, info) => {
    // paremeter validations
    await Promise.all(Object.keys(action['inputs'] || {}).map(param => {
      if (inputs[param]) {
        if (typeof inputs[param].custom === 'function') {
          return inputs[param].custom(args[param])
        }
      }
    }))
      .catch(err => {
        return flaverr({ code: 'E_INVALID_ARGINS' }, err)
      })
